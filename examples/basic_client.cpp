...
int main(int argc, char* argv[]) {
    ...
    auto handler = bedrock::network::ConnectionHandler::createClient();
    if (!handler->start()) {
    ...
    bedrock::raknet::SystemAddress server_addr;
    ...
    while (true) {
        ...
        for (auto& evt : events) {
            switch (evt.type) {
                case bedrock::network::ConnectionEvent::Type::Connected: {
                    std::cout << "Connected to server!" << std::endl;
                    connected = true;
                    server_addr = evt.address;
                    
                    // Send RequestNetworkSettings
                    bedrock::RequestNetworkSettingsPacket req;
                    req.client_network_version = bedrock::PROTOCOL_VERSION;
                    handler->sendPacket(server_addr, req);
                    std::cout << "Sent RequestNetworkSettings (protocol=" 
                              << bedrock::PROTOCOL_VERSION << ")" << std::endl;
                    break;
                }
                
                case bedrock::network::ConnectionEvent::Type::Disconnected: {
                    std::cout << "Disconnected from server" << std::endl;
                    handler->shutdown();
                    return 0;
                }
                
                case bedrock::network::ConnectionEvent::Type::PacketReceived: {
                    std::cout << "Received packet: " 
                              << bedrock::packetIdToString(evt.packet_id) 
                              << " (" << static_cast<int>(evt.packet_id) << ")"
                              << " [" << evt.raw_data.size() << " bytes]" << std::endl;
                    
                    // Handle specific packets
                    switch (evt.packet_id) {
                        case bedrock::PacketId::NetworkSettings: {
                            bedrock::NetworkSettingsPacket settings;
                            ...
                            // Apply compression
                            auto* bc = handler->getBedrockConnection(server_addr);
                            if (bc) {
                        ...
                        case bedrock::PacketId::PlayStatus: {
                            bedrock::PlayStatusPacket status;
                        ...
                        case bedrock::PacketId::Disconnect: {
                            bedrock::DisconnectPacket disc;
