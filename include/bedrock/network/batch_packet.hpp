// =============================================================================
// BedrockProtocol - Batch Packet Processing
...
public:
    ...
    std::vector<uint8_t> encodeSingle(const bedrock::Packet& packet) {
        // Serialize packet into sub-packet format
        auto sub_packet = bedrock::PacketCodec::encodeSingle(packet);
    ...
    std::vector<uint8_t> encodeBatch(const std::vector<const bedrock::Packet*>& packets) {
        auto batch = bedrock::PacketCodec::encodeBatch(packets);
    ...
    std::vector<std::pair<PacketId, std::vector<uint8_t>>> decode(
        ...
        // Decode sub-packets
        bedrock::PacketCodec::decodeBatch(decompressed.data(), decompressed.size(),
            [&result](const bedrock::PacketCodec::DecodedSubPacket& sub) {
    ...
    void decode(const std::vector<uint8_t>& payload,
        ...
        bedrock::PacketCodec::decodeBatch(decompressed.data(), decompressed.size(),
            [&callback](const bedrock::PacketCodec::DecodedSubPacket& sub) {
