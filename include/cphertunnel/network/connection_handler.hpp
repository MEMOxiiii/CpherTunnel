// =============================================================================
// CpherTunnel - Connection Handler  
...
struct ConnectionEvent {
    ...
    cphertunnel::PacketId packet_id = PacketId::Login;
    std::vector<uint8_t> raw_data;
};

// =============================================================================
// Connection - Full protocol stack for one connection
// =============================================================================
class Connection {
public:
    ...
    explicit Connection(const raknet::SystemAddress& remote)
    ...
    std::vector<uint8_t> encodePacket(const cphertunnel::Packet& packet) {
...
public:
    ...
    bool sendPacket(const raknet::SystemAddress& addr, const cphertunnel::Packet& packet) {
        auto* bc = getConnection(addr);
        if (!bc) return false;
    ...
    bool sendBatch(const raknet::SystemAddress& addr,
                   const std::vector<const cphertunnel::Packet*>& packets) {
        auto* bc = getConnection(addr);
        if (!bc) return false;
    ...
    void broadcastPacket(const cphertunnel::Packet& packet) {
        ...
        for (auto& evt : raknet_events) {
            switch (evt.type) {
                case raknet::PeerEvent::Type::Connected: {
                    ...
                    auto bc = std::make_unique<Connection>(evt.address);
                    connections_[evt.address.toString()] = std::move(bc);
                ...
                case raknet::PeerEvent::Type::Disconnected: {
                    connections_.erase(evt.address.toString());
                ...
                case raknet::PeerEvent::Type::GamePacket: {
                    auto* bc = getConnection(evt.address);
                    if (bc) {
    ...
    Connection* getConnection(const raknet::SystemAddress& addr) {
        auto it = connections_.find(addr.toString());
